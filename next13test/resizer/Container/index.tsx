...
//React.Pure Component only compares objects shallowly
...
class Container extends React.Component<Props> {
  ...
  //Going to try to implement this with resizeobserver
  /*private resizeObserver = new ResizeObserver(entries => {
    console.log
  }) */
  ...
  //Returns context that will be passed to the container
  ...
  //Return all styled props excluding those that redefine
  ...
  //This function is called in bar and section
  //They call it from the passed in context
  //Then Container get's bar and section's passed in props
  //Pushes them into it's own array
  //And returns an id for the bar/section based on the length
  //of the array of childProps it has
  private createID = (childProps: ChildProps) => {
    this.childrenProps.push(childProps);
    return this.childrenProps.length - 1;
  };

  //Called in bar and section from passed in context
  //
  private populateChildInstance = (
    id: number,
    ref: React.RefObject<HTMLElement>,
  ) => {
    console.log(`Creating a new child instance from id: ${id}`);
    console.log("Current childrenProps");
    console.log(this.childrenProps)
    if (ref.current) {
  ...
  private makeSizeInfos(): SizeRelatedInfo {
    ...
    let prevRect: number;
    console.log(this.childrenProps.length);
...
export { Container, Resizer, StyledContainer}
